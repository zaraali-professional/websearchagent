...
# LangChain LLM (Groq - OpenAI compatible)
llm = ChatOpenAI(
...
search_tool = Tool(
    ...
    description="Useful for web-based factual research."
)

# Agent instructions (same as before)
instructions = """
...
with ui.card().classes('w-full max-w-3xl mx-auto mt-8 bg-gradient-to-b from-gray-50 to-white'):
    ...
    async def handle_search():
        ...
        try:
            ...
            # Run agent with prefixed instruction
            result = await loop.run_in_executor(None, lambda: web_search_agent.run(f"{instructions}\n\n{query}"))

            response_area.clear()
            with response_area:
                ui.markdown(result).classes('w-full animate-fade-in')
